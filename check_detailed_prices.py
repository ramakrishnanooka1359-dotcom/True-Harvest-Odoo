...
def check_detailed_prices():
    ...
    for t in templates:
        ...
        variants = models.execute_kw(
            ...
            {'fields': ['id', 'display_name', 'lst_price', 'price_extra', 'variant_price_extra']}
        )
        ...
        for v in variants:
            ...
            print(f"    variant_price_extra: {v.get('variant_price_extra')}")
            
            # Check the PTAVs for this variant
            ptav_ids = models.execute_kw(
                ODOO_DB, uid, ODOO_API_KEY,
                'product.template.attribute.value', 'search_read',
                [[('id', 'in', models.execute_kw(ODOO_DB, uid, ODOO_API_KEY, 'product.product', 'read', [[v['id']], ['product_template_attribute_value_ids']])[0]['product_template_attribute_value_ids'])]],
